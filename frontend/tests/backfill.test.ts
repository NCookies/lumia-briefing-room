...
test('progress text tells the scan phase from the game phase', () => {
  assert.equal(describeProgress(running({ phase: 'scan', message: '77분 지점' })), '녹화를 훑는 중 · 세션 1/2 · 77분 지점')
  assert.equal(
    describeProgress(running({ phase: 'process', gamesDone: 2, gamesTotal: 5, clips: 14, message: '09-24 15:55 경기' })),
    '경기 분석 중 · 3/5판 · 클립 14개 · 09-24 15:55 경기',
  )
})

test('progress text before anything is known is calm', () => {
  assert.equal(describeProgress(running({ phase: 'scan', sessionIndex: 0, sessionTotal: 0, message: '' })), '녹화를 훑는 중')
})

test('the finished summary names what was made and what was skipped and why', () => {
  const lines = describeResult(result())
  assert.equal(lines[0], '경기 3판에서 클립 20개를 만들었습니다.')
  assert.ok(lines.some((l) => l.includes('이미 클립이 있는 경기 1판')))
  assert.ok(lines.some((l) => l.includes('앞부분이 이미 지워진 경기 1판')))
})
...
test('nothing new is said plainly', () => {
  ...
  assert.equal(lines[0], '새로 만들 경기가 없습니다.')
})
...
test('cancelled summary keeps what was already made and says it can resume', () => {
  ...
  assert.equal(lines[0], '취소했습니다. 그때까지 끝낸 경기 2판(클립 11개)은 그대로 남아 있습니다.')
...
test('failures and give-ups are mentioned', () => {
  ...
  assert.ok(lines.some((l) => l.includes('실패한 경기 1판')))
  assert.ok(lines.some((l) => l.includes('아직 진행 중인 경기 1판')))
  assert.ok(lines.some((l) => l.includes('여러 번 실패해 건너뛴 경기 2판')))
})
