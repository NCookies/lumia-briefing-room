...
export function AdminPanel({ active }: { active: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      setSummary(await getAdminSummary(mode, forceRefresh))
  ...
  }, [mode, query, kind, offset, forceRefresh])
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-zinc-500">서버에 쌓인 라벨(개발 모드 전용). 관리자 토큰은 저장소 루트 .env 의 LUMIA_ADMIN_TOKEN 을 씁니다.</p>
        <div className="flex items-center gap-2">
          <select
            className={inputClass}
            value={mode}
            onChange={(e) => {
              setMode(e.target.value as AdminMode)
              setOffset(0)
            }}
          >
          ...
          <button
            type="button"
            className="rounded border border-zinc-600 px-2 py-1 text-sm text-zinc-300 hover:bg-zinc-700 disabled:opacity-50"
            disabled={loading}
            onClick={() => {
              setOffset(0)
