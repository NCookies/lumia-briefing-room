...
export function describeProgress(status: BackfillStatus): string {
  ...
  if (status.phase === 'process') {
    return [
      '경기 분석 중',
      `${(status.gamesDone ?? 0) + 1}/${status.gamesTotal ?? 0}판`,
      `클립 ${status.clips ?? 0}개`,
      ...extra,
    ].join(' · ')
  }
  const session = status.sessionTotal ? [`세션 ${status.sessionIndex}/${status.sessionTotal}`] : []
  return ['녹화를 훑는 중', ...session, ...extra].join(' · ')
}
...
export function describeResult(result: BackfillResult): string[] {
  ...
  if (result.cancelled) {
    lines.push(
      `취소했습니다. 그때까지 끝낸 경기 ${result.gamesProcessed}판(클립 ${result.clipsCreated}개)은 그대로 남아 있습니다.`,
    )
    lines.push('다시 시작하면 이어서 진행합니다. 이미 훑은 녹화는 다시 보지 않습니다.')
  } else if (result.gamesProcessed === 0) {
    lines.push('새로 만들 경기가 없습니다.')
  } else {
    lines.push(`경기 ${result.gamesProcessed}판에서 클립 ${result.clipsCreated}개를 만들었습니다.`)
  }
  const skipped = result.skipped
  if (skipped.known > 0) lines.push(`이미 클립이 있는 경기 ${skipped.known}판은 건너뛰었습니다.`)
  if (skipped.cutAtStart > 0) {
    lines.push(`앞부분이 이미 지워진 경기 ${skipped.cutAtStart}판은 건너뛰었습니다(스팀이 오래된 녹화를 지웁니다).`)
  }
  if (skipped.stillRunning > 0) lines.push(`아직 진행 중인 경기 ${skipped.stillRunning}판은 끝난 뒤 자동으로 만들어집니다.`)
  if (result.gamesFailed > 0) lines.push(`실패한 경기 ${result.gamesFailed}판은 다시 시작하면 한 번 더 시도합니다.`)
  if (skipped.gaveUp > 0) lines.push(`여러 번 실패해 건너뛴 경기 ${skipped.gaveUp}판이 있습니다. 진단 정보를 보내 주세요.`)
  return lines
}
