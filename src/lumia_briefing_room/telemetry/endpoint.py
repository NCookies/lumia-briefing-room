...
→ 번들 파일(`data/telemetry_endpoint.json`, tools/build_release.py 가 환경변수에서 만든다). 토큰이 없으면 전송 기능은 꺼진 것과 같다.
"""
...
DEFAULT_SERVER_URL = "https://server.example.invalid"
...
def load_endpoint(cfg, environ=None) -> Endpoint | None:
    ...
    if not isinstance(token, str) or not token.strip():
        return None
    url = cfg.telemetry.server_url or environ.get(URL_ENV) or bundled.get("url") or DEFAULT_SERVER_URL
    return Endpoint(url=str(url).rstrip("/"), token=token.strip())
