...
usage:
    python tools/build_regions.py <frames_dir> <labels.jsonl> <output.npz> [--score white]
...
from lumia_briefing_room.detect.day import white_score  # noqa: E402
...
def build_region_templates(
    samples: dict[str, list[np.ndarray]], score=region_score
) -> dict[str, np.ndarray]:
    ...
    for name, imgs in samples.items():
        ...
        templates[name] = build_region_template([score(img) for img in imgs])
    return templates
...
def main() -> None:
    ...
    parser.add_argument(
        "--score", choices=["max", "white"], default="max",
        help="max: 색과 무관(지역명, 글자색이 바뀜) / white: 흰 글자만(일차, 색 배경에 반응 안 함)",
    )
    args = parser.parse_args()

    samples = load_labeled_region_samples(args.frames_dir, args.labels)
    templates = build_region_templates(samples, white_score if args.score == "white" else region_score)
    if not templates:
