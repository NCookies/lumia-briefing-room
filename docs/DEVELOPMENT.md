...
- **관리자 화면** (개발 모드 전용 "관리자" 탭, 별도 프로그램 없음): `run.bat`/`dev.bat` 으로 소스 실행하면 클립 탭 옆에 "관리자" 탭이 보인다(배포 빌드에는 탭도 API `/api/admin/*` 도 없다 — 서버가 요청마다 개발 모드인지 확인해 404). 저장소 루트 `.env` 의 `LUMIA_ADMIN_TOKEN`·`LUMIA_RECEIVER_URL`(또는 같은 이름의 환경변수)을 앱 프로세스가 읽어 서버 `GET /v1/admin/labels` 로 라벨을 받아 라벨 수·종류·앱 버전·해상도·수신 일자·설치별 집계와 `installId`(앞부분)/`clipKey` 검색을 보여 준다. 토큰은 앱 서버만 갖고 브라우저에는 넘기지 않는다. `.env` 는 요청 때마다 읽으므로 고친 뒤 앱을 다시 띄울 필요 없이 "새로고침"만 누른다(서버 응답은 캐시). 값 뒤 ` # 설명` 은 주석으로 잘라내고, 서버 주소에 `https://` 가 없으면 붙인다(앱의 라벨·로그 전송도 같다). 토큰에 한글·공백이 섞이면 500 이 아니라 안내 문구가 든 503 이다. release/dev 선택이 서버의 `mode` 다. 서버 상태·오류 로그·진단 번들은 서버에 읽기 API 가 없어 아직 보여 주지 못한다([plan-infra §8](plan-infra.md)).
