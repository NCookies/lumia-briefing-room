...
- [x] I4. **이 저장소 쪽 전송 클라이언트 연결 (2026-09-25, plan-deploy D10)** — 앱→서버 전송, 서버→로컬 가져오기(`tools/pull_labels.py`), 통합 테스트(`pytest -m integration`, 로컬 도커의 receiver 컨테이너)까지. 실서버에는 새 이미지·`ADMIN_TOKEN` 반영이 남았다
