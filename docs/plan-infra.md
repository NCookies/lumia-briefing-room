...
- 항목 5(개인정보): 보관 기간·IP 처리는 [privacy.md](privacy.md) 대로 서버에 구현(접근 로그 비활성, 로그·진단 90일 자동 삭제). **배포는 했고 접근 로그에 IP 가 없는 것까지 확인했다.** 인터넷 스캐너가 만드는 TLS 오류 기록에 IP 가 남는지와 90일 자동 삭제의 실제 동작(오래된 파일이 생겨야 관측 가능)은 아직 관측하지 못했다 — 그래서 [privacy.md](privacy.md) 는 그 부분을 "IP 가 남을 수 있고 크기 기준으로 순환된다"고만 적어 2026-09-25 에 확정했다.
- 항목 6·7·8: 아직 미정.

## 8. 추후 구현 (아직 만들지 않는다)

- **관리자 전용 대시보드(로컬 실행 전용).** 서버 상태, 쌓인 라벨링 데이터(라벨 수·환경별 분포·`installId`/표시용 ID 로 검색), 오류 로그·진단 번들(접수 번호로 조회)을 한 화면에서 보는 운영자용 화면이다. 공개 사용자에게는 노출하지 않는다.
  - **접근 방식은 정하지 않았다.** 후보: (1) 내 PC 에서 로컬로 띄우고 서버의 관리자 조회 API 를 별도 관리자 토큰으로 호출, (2) SSH 터널로 서버의 대시보드에 접속(서버는 `localhost` 바인드만), (3) 서버 데이터를 내려받아 로컬에서 조회(이미 `tools/pull_labels.py` 로 라벨을 가져오는 경로가 있다). 수신 API 와 같은 프로세스·같은 토큰을 쓰지 않는다(관리자 권한이 앱에 들어가는 토큰과 섞이면 안 된다).
