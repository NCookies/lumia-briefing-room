...
- **관리자 전용 대시보드(로컬 실행 전용).** 서버 상태, 쌓인 라벨링 데이터(라벨 수·환경별 분포·`installId`/표시용 ID 로 검색), 오류 로그·진단 번들(접수 번호로 조회)을 한 화면에서 보는 운영자용 화면이다. 공개 사용자에게는 노출하지 않는다.
  - **1단계 구현(2026-09-25)**: 후보 (1) 로 정했다. 앱과 별개 진입점 `admin.bat`(`tools/admin_dashboard.py`)이 로컬 127.0.0.1 에서 관리자 토큰으로 `GET /v1/admin/labels` 를 받아 라벨 집계·검색을 보여 준다(토큰은 브라우저에 넘기지 않음). **남은 것(서버 저장소에 읽기 API 추가 후)**: 서버 상태, 오류 로그, 진단 번들(접수 번호 조회).
