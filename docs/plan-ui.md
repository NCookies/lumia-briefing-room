...
- [ ] **자동 정리 후에도 게임 기록 유지** — 게임 행은 클립 메타데이터를 묶어 그리는 것이라 클립이 다 지워지면 행도 사라진다. 영상 정리와 전적 기록을 분리한다: 클립을 지울 때(수동 완전 삭제·휴지통 비우기·유예 만료·자동 정리 4경로 공통, 라벨 보관소와 같은 지점) 그 경기의 요약(`sessionDir`·`matchStartUtc`·모드·`matchResult`(순위·캐릭터·TK/K/A·팀원 캐릭터)·닉네임)을 `clips/.games/<경기키>.json` 에 남기고, 결과표 이미지도 남긴다(1280px JPEG 1장, 약 100~200KB). 목록 API 는 클립 없는 경기를 이 요약에서 읽어 게임 행으로 함께 내려주고(`clipCount: 0`), 화면은 "클립 삭제됨" 표시의 접힌 행(결과표 썸네일은 유지, 클립 카드 없음)으로 그린다. 게임 단위 삭제 버튼은 기록까지 지운다(확인창에 "게임 기록도 함께 삭제됩니다" 명시). 자동 정리 화면에 체크박스 "게임 기록은 유지"(`retention.keepGameRecords`, 기본 `true`)를 둔다 — 끄면 클립이 다 사라진 경기의 요약·결과표 이미지도 지운다(현재 동작). 다시 분석하면 새 클립이 요약을 대체한다. 순수 로직·API 는 TDD
- [ ] **브라우저 디버깅 환경 구축** — §6 참고
...
- [ ] `POST /api/client-log` + 로그 파일 기록(테스트: 요청 → 로그 파일에 `[client]` 줄)
- [ ] 프론트 전역 오류 핸들러(중복 억제·재전송 없음) + `npm run build`
- [ ] Playwright MCP 연결 및 실측(Edge, HEVC 재생, 콘솔 읽기)
- [ ] README "실행 방법"에 §9 브라우저 디버깅(로그 파일 위치, MCP 연결 명령) 추가 — 구현과 같은 커밋에서
