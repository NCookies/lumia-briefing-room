...
- [ ] **자동 정리 화면을 기준별 체크박스로 개편** — 지금은 "자동 정리 켜기" 하나와 비우면 제한 없음인 숫자칸 3개(경기 후 N일·N개·N GB). 개편: 기준마다 체크박스(`N일이 지난 클립` / `N개 초과` / `N GB 초과`)를 두고 체크해야 해당 숫자칸이 활성화되며, 체크 해제 = 그 기준 미적용(`null`)으로 저장. 맨 위 "자동 정리 켜기"는 유지하고 꺼지면 아래를 비활성으로 보인다. 설정 키(`retention.*`)와 백엔드는 그대로, `CleanupPanel.tsx` 만 바꾼다(저장 시 체크 안 된 기준은 `null`). 미리보기·즉시 실행·보호 항목은 그대로
