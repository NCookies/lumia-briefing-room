...
- [ ] **볼륨 설정이 재실행 후 초기화되는 문제 수정** — 볼륨/음소거는 이미 localStorage 에 저장하지만(`frontend/src/volume.ts`), localStorage 는 origin(포트 포함)별이라 `ui.port=auto`(트레이 경로 `resolve_port`)가 실행마다 다른 포트를 고르면 저장값이 안 보인다. 해결 후보: (a) `ui.port` 기본을 고정 포트로(점유 중이면 다음 빈 포트로 폴백), (b) 볼륨을 서버 설정(`player.volume`/`player.muted`, `PUT /api/config`)에 저장. 다른 브라우저 기억값(선택 탭·"다시 확인하지 않기" 등)도 같은 원인이므로 (a) 를 우선 검토. 쿠키는 origin(포트) 무관하지만 서버 설정이 더 단순해 쓰지 않는다. 결정은 SPEC §7.7 에 반영
