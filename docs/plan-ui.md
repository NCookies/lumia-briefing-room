...
- [x] 계획 수립 (이 문서)
- [ ] **다시보기 탭** (설계만, 미구현 — [plan-vod.md §3](plan-vod.md), [SPEC §2.14](SPEC.md)) — 헤더 상단 탭 `내 녹화 | 다시보기` 로 분리(선택 탭은 localStorage, 탭별 필터·정렬·펼침 독립), 다시보기 탭은 영상 단위 접이식 섹션(`VodSection`) 안에 기존 `GameSection`(시각 표시만 영상 내 위치로) 재사용, 분석 시작/취소/진행률, 옵션 모달 "다시보기" 탭(영상 경로 추가·스트리머 표시명). 기존 화면은 "내 녹화" 탭으로 동작 변화 없이 이동
