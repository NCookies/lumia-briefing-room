...
### 4-3. `lumia-briefingroom.localhost` 주소 — 확인 필요 (아직 실측 안 함)

위 §0 "접속 주소" 항목의 전제 중 실측으로 확인하지 못한 것. 구현 전에 확인하고, 결과를 이 절에 적어 ✅ 로 바꾼다.

- **pywebview(Edge WebView2)가 `*.localhost` 를 루프백으로 여는가.** Chromium 은 그렇게 동작하는 것으로 알려져 있으나 이 프로젝트의 WebView2 로 직접 열어본 적은 없다. 안 열리면 `ui.shell=webview` 만 `127.0.0.1` 을 쓰는 예외를 둔다.
- **`127.0.0.1` 에만 바인드한 서버에 `::1`(IPv6) 로 접속을 시도하는 문제.** `*.localhost` 는 IPv4·IPv6 루프백 둘 다로 풀릴 수 있다. 브라우저가 다른 쪽으로 넘어가 접속하는지, 지연이 없는지 확인한다.
- **80번 바인드가 일반 권한으로 되는가.** Windows 에서는 보통 되지만 IIS·`http.sys`·백신·VPN 등이 이미 잡고 있으면 실패한다. 이 PC 에서의 실측과 실패 시 폴백 동작(포트가 붙은 주소로 열기)을 확인한다.
