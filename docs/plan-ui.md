...
- **저장 버튼을 윈도우 내장 대화상자로.** 지금은 웹 UI 의 자체 폴더 선택기(`api/export.py`, 저장 대화상자)라 불편하고 OS 와 이질적이다. pywebview 창에서는 네이티브 저장/폴더 대화상자(`webview.create_file_dialog`)로, 브라우저 실행 경로에서는 서버가 tkinter/Win32 대화상자를 띄우는 방식 등을 비교해야 한다. 브라우저·트레이 경로에서 어떻게 할지가 확인 필요.
