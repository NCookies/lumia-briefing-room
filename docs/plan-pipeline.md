...
- [x] **감시를 줄 단위 tail 에서 주기적 폴링으로 교체 (2026-09-22)** — 앞의 `tail_follow`/`run_forever`/`run_once` 서술은 이 항목으로 대체됐다(코드 삭제). 실제 사고 두 건이 원인: ① `tail_follow` 가 처음 연 `Player.log` 핸들을 쥐고 있어 게임이 재실행돼 로그가 바뀌면 새 경기를 못 봤다(23:41 앱 시작, 23:43 게임 실행, 23:48~00:02 경기 → 클립 없음). ② 앱 시작 직후 밀린 경기를 처리하는 11분 동안 시작한 경기(01:33)를, 처리 뒤에 시작한 감시가 '지금 끝'부터 읽어 시작 줄을 못 보고 종료 줄(01:54)을 무시했다. 이제 `pipeline/watcher.run_polling` 이 `poll_interval_ms`(기본 1초)마다 `Player.log`/`Player-prev.log` 를 통째로 다시 읽어 끝난 지 `delaySec` 이 지난 처리 안 된 경기를 오래된 것부터 처리한다(처리 이력이 중복 방지, 세션 없는 경기는 건너뜀, 예외로 실패한 경기는 3번까지만 재시도해 감시 스레드가 죽지 않음, `--once` 는 남은 게 없으면 종료). 실제 로그+처리 이력으로 점검: 처리 안 된 경기는 놓친 01:33~01:54 하나뿐이고 새 방식이 잡아낸다. 테스트 7건
