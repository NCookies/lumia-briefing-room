...
**게임 결과(순위·랭크 여부)**: 경기가 끝날 때 나오는 결과 화면(`4/7 실험 종료`)을 OCR 로 읽어 클립 메타데이터의 `matchResult`(`matchType` rank/normal, `placement`, `total`, `outcome`, `nickname`)에 넣고, 게임 섹션 제목에 `랭크 · 4위 / 7팀 · 실험 종료` 로 표시한다. 원본 녹화가 남아 있는 새 경기부터 채워지고, 이미 저장된 클립은 결과 화면이 영상에 없어 비어 있다. 닉네임은 첫 결과 화면에서 자동으로 읽어 설정에 저장하고(비어 있을 때만), 헤더 "⚙ 옵션" 에서 직접 고칠 수 있다.
